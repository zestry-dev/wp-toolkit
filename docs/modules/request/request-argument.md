...
A route, an ability and an AJAX action all declare their input this way, but WordPress does different amounts of the work on each:
...
The same door takes everything else an attribute cannot hold: `'enum' => get_post_types()` is a function call, and a function call is not a constant expression either. **An AJAX action has no equivalent and needs none** — nothing publishes its schema, so a rule you would have stated there belongs in `validate`.
