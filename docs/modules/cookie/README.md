...
> [!IMPORTANT]
> **A module is built because `bootstrap.php` lists it.** `Cookie` does nothing until something asks, so it goes at the top level — which `wp zt add` writes for you. Left out, the first `with( Cookie::class )` throws rather than building it, which [`wp zt doctor`](../../commands/doctor.md) catches before a request does.
...
`Cookie` takes no configuration. The entry above is all it needs — reach it with `$this->with( Cookie::class )` from any module or discovered file, or `$plugin->get( Cookie::class )` from your entry file.
