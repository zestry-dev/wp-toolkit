...
A discovered file's name is the thing it registers as: `resources/commands/greet.php` is `wp your-plugin greet`, `post-types/book.php` is the `book` post type, `fields/acme_rating.php` is the `acme_rating` meta key. You never repeat that name inside the file.
...
| [`cli`](cli/) | add a `wp` command | `resources/commands/` | [`Command`](cli/command.md) | `path` |
| [`cookie`](cookie/) | read and write a cookie, and carry a value across a redirect | — | — | `transients` |
| [`cron`](cron/) | run something on a schedule | `resources/schedules/` | [`Schedule`](cron/schedule.md) | `path` |
| [`db`](db/) | name a database table, yours or WordPress's | — | — | — |
| [`fields`](fields/) | register post meta, and render it on the editor | `resources/fields/` | [`Field`](fields/field.md) | `path` |
