...
> [!IMPORTANT]
> **A module is built because `bootstrap.php` lists it, and the heading says when.** `Assets` acts the moment it is built, so it goes under the hook it acts on — which `wp zt add` writes for you. Left at the top level it throws; left out entirely, nothing is discovered and nothing reports why, which is what [`wp zt doctor`](../../commands/doctor.md) catches.
...
`Assets` takes no configuration. The entry above is all it needs — reach it with `$this->with( Assets::class )` from any module or discovered file, or `$plugin->get( Assets::class )` from your entry file.
