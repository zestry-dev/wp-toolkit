...
> [!IMPORTANT]
> **A module is built because `bootstrap.php` lists it.** `Path` does nothing until something asks, so it goes at the top level — which `wp zt add` writes for you. Left out, the first `with( Path::class )` throws rather than building it, which [`wp zt doctor`](../../commands/doctor.md) catches before a request does.
...
`Path` takes no configuration. The entry above is all it needs — reach it with `$this->with( Path::class )` from any module or discovered file, or `$plugin->get( Path::class )` from your entry file.
