...
Requires `wp zt init` to have already run in this plugin. Like `make module`, there is no fixed conventional directory to default to, so it goes in your own `{zestry.json root}/Services/` directory — beside the copied `Core/Services/` rather than inside it, since `Core/` is what `wp zt update` may replace and nothing you write belongs there.
...
A **module** does. It binds a hook, registers a post type, walks a directory. Because it acts on its own it has to be built for that to happen, so it is listed in `bootstrap.php` and the plugin builds it as the plugin loads. `wp zt make module` generates that shape, with the `on_boot()` the base class requires.
