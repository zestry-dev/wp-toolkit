<!--
    Generated from resources/commands/make/post-type.php.
...
- **`<name>`**  
  The local name, e.g. 'book'. Becomes both the filename (`{name}.php`) under `resources/post-types/` and the registered post type itself — unlike every other `make` type, this name is NOT namespaced to the plugin slug (WordPress caps a post type name at 20 characters), so pick something short and globally unique.
