<!--
    Generated from resources/commands/make/migration.php.
...
The Migrations module discovers it, but unlike every other discovery type this one does not run itself. The module reads your `resources/migrations/` directory in filename order and runs each `Migration` at most once per site, when something asks it to: `wp {slug} migrations run`, or a `run_pending()` call from whatever trigger fits your release process.
