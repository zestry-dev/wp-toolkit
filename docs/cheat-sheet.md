...
- **A fresh install runs every migration you ever wrote** — forty of them, each diffing the same tables through `dbDelta()` to reach a schema the last one could have created outright. `wp {slug} migrations squash` dumps that schema as a single `Baseline`, which a fresh install runs *instead of* everything before it. A site with a history ignores it and keeps migrating one step at a time.
- **Squash is an authoring command, not a deployment one.** It reads a database and writes a file you review and commit; nothing regenerates it later, and no build ever needs MySQL for it. Forgetting to re-squash is safe — newer migrations still run one by one, so a stale baseline is slower, never wrong. `migrations squash --check` reads files only and exits non-zero when the baseline has fallen behind, which is the CI half.
- **A baseline carries schema and only schema.** Everything it subsumes is recorded as run *without running*, so a migration that also seeded an option or inserted a row stops doing that on fresh installs — move anything non-schema into the baseline's own `up()`.
- **There is one `resources/migrations/baseline.php`,** rewritten in place by each squash, and `Baseline::subsumes()` names exactly what it replaces. The list is the mechanism, not a note: a migration it does not name still runs, however its filename sorts.
...
| [`wp zt tests`](commands/tests.md) | Writes the PHPUnit suite: `phpunit.xml.dist`, `tests/bootstrap.php`, `tests/Support/TestCase.php`, the WP-CLI doubles, an example test, `.wp-env.test.json`, and the composer/npm entries that run them. `--no-wp-env`, `--yes` |
...
| [`update`](commands/make-update.md) | `lib/Modules/` | an update handler, **declared** too |
| [`abstract`](commands/make-abstract.md) | `lib/Abstracts/` | a base your own files share. `--for=<type>`, `--extends=` |
| [`test`](commands/make-test.md) | `tests/Integration/` | a PHPUnit test, `Test`-suffixed. Needs [`wp zt tests`](commands/tests.md) first |
...
**6 of the 23 generators ask for what you leave out** — [`block`](commands/make-block.md), [`field`](commands/make-field.md), [`post-type`](commands/make-post-type.md), [`route`](commands/make-route.md), [`shared`](commands/make-shared.md), [`taxonomy`](commands/make-taxonomy.md). Give every option and none of them stops. The other 17 take no options they could ask about — but *any* generator stops to ask before overwriting a file, or to offer the module the generated file needs. `--yes` answers all of it without reading input, which is what an unattended run wants.
<!-- /zestry:include -->
