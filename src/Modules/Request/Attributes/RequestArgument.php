...
/**
 * Request API: RequestArgument attribute
 */
 ...
 *
 * One attribute for both callable surfaces this toolkit registers — a
 * {@see \Zestry\WPToolkit\Modules\RestApi\RestRoute} and an
 ...
 * {@see \Zestry\WPToolkit\Modules\Request\Request} module builds the route's `args` or the
 ...
 * or `$input['order_id']`.
 *
 * **The property states both the type and whether it is required**, since PHP
 * already made you declare both. `int` is `integer`, `float` is `number`, `bool`
 * is `boolean`, and `?int` accepts null as well. An argument with no default is
 * required — leaving it optional would mean a handler reading an uninitialized
 * property, which is a PHP error rather than a missing value — and one with a
 * default is optional, with that default published so a caller knows what
 * leaving the key out gets them.
 *
 * If you have written DTOs, value objects or form objects before, this is that:
 * your route or ability *is* the data object, its properties are the fields, and
 * the values arrive already checked and already the right type. What differs is
 * that the declaration is also *published* — WordPress turns it into the schema
 * a client, or an AI agent, reads to work out what to send.
 *
 * ## How it works
 *
 ...
 *
 * Nothing reaches step 4 that failed an earlier one. That is the whole point of
 * declaring rather than reading: the checking happens where a caller can see it
 * coming.
 *
 ...
 *
 * Anything else is refused at registration, with a message naming the property.
 * See **Limitations** below.
 *
 * Two exceptions to the required/optional rule, both on a route: a `{token}` in
 * the URL pattern is **always required**, whatever its property says, since
 * there is no optional path segment; and an upload is checked here rather than
 * by WordPress, because it never appears among a request's parameters.
 *
 * ## Dates
 *
 ...
 * interface itself gives you a `DateTimeImmutable`, since a value that cannot
 * change underneath its holder is the safer of the two.
 *
 * What a caller has to send is a full date and time: `2026-08-04T12:00:00Z`, an
 * offset like `+10:00`, or `2026-08-04 12:00:00`. A bare `2026-08-04` is
 * refused, and so is anything WordPress cannot read as a moment.
 *
 * **A time with no offset is read as UTC**, because WordPress runs PHP in UTC —
 ...
 * ## Enums
 *
 * A property typed as an enum is a closed set, and the schema says so — the
 ...
 * not change what callers must send. A pure enum has none, so its case names
 * stand in — the schema lists them either way, so a caller never has to know
 * which kind it is looking at.
 *
 * This is the better way to write `schema: array( 'enum' => ... )`: the set
 * lives in one place, and the handler reads a case rather than a string.
 *
 * ## Structures
 *
 ...
 * publishing a shape that says nothing.
 *
 * A class used this way needs nothing but public typed properties carrying this
 * attribute — no base class, no constructor. It is built without calling one,
 * so property defaults apply and required constructor arguments are not a
 * problem.
 *
 * ## Files
 *
 ...
 * the shape is known: a caller can read a structure and cannot read an open
 * object.
 *
 ...
 * the same rule in a callback is one it can only discover by getting it wrong.
 * For an AI agent choosing what to send, that is the difference between a
 * contract and a guessing game.
 *
 * `validate` and `sanitize` are for what JSON Schema cannot say — that an id
 * exists, that a date has not passed, that a slug is free:
 *
 * ```
 ...
 * accepts a numeric string for an `integer`, so `"42"` is a valid thing for a
 * caller to send. It arrives as `42` either way.
 *
 * ## Translation, and anything else computed
 *
 * `__()` cannot go inside the attribute. PHP allows only constant expressions in
 * an attribute argument, so this fails when the file is compiled, before
 ...
 * ```
 *
 * State that one argument's description in `input_schema()` on an ability, or
 * `args()` on a route. What you write there is merged *over* the schema your
 * declarations already give, so the property keeps its type, its required-ness,
 * its `validate` rule and its binding — you are finishing the sentence the
 * attribute started, not writing the schema instead of it:
 *
 * ```
 ...
 * ```
 *
 * A route says the same through `args()`, keyed by argument name rather than
 * nested under `properties`, because that is the shape `register_rest_route()`
 * takes. A keyed map is merged into; a list — `required`, an `enum`, a nullable
 * `type` — is replaced whole, so state all of it when you state any of it.
 *
 ...
 * ## Limitations
 *
 * Each is refused with a message naming the property, and never silently. All
 * but the last two are caught while your route or ability registers, so you meet
 * them the first time the code loads rather than the first time someone calls.
 *
 ...
 * ### Why not readonly
 *
 * PHP lets a `readonly` property be assigned exactly once. Your route or ability
 * object is built **once** and answers every call for the rest of the request,
 * so binding arguments onto it means assigning the same properties again on the
 * next call — the assignment PHP refuses. On an ability it fails sooner still:
 * values are bound before `permission_check()` and again before `handle()`.
 *
 * A structure is the opposite. It is built fresh every time, so its properties
 * are assigned exactly once in their lifetime, which is what `readonly` asks
 * for. If immutable arguments are what you are after, that is the shape:
 *
 * ```
 * final class Filter {
 *
...
final class RequestArgument {

	/**
	 * Validation runs first and sanitising second, which is the order WordPress
	 * dispatches a route's own callbacks in (`has_valid_params()` then
	 * `sanitize_params()`) and the safe one: you decide about the value that was
	 * actually sent, rather than about a cleaned-up version of it.
	 *
	 * Both are called with the value alone, though WordPress hands its own
	 * callbacks three arguments. A callback here answers one question about one
	 * value, which is all `is_email` or `sanitize_text_field` want — and passing
	 * more is what makes the obvious `'intval'` fatal, since it is declared
	 * `intval( $value, $base = 10 )` and would take the request as its base.
	 *
	 * A check spanning several arguments belongs in the handler, where every
	 * bound property is available at once and the error can name the combination
	 * that was wrong. A callback here sees one value, in isolation.
	 *
	 * @param string               $description What this argument is, in a sentence a stranger can act on.
	 *                                          Optional, for an argument whose name and type already say it
	 *                                          — but the caller has nobody to ask, so write one wherever
	 *                                          the name alone would leave it guessing.
	 * @param callable|null        $validate    Called as `( $value )` for a rule JSON Schema cannot express;
	 *                                          returning false rejects the call. A bare built-in name, a
	 *                                          `[Class, 'method']` pair or a one-parameter closure.
	 * @param callable|null        $sanitize    Called as `( $value )` once validation passes; its return value
	 *                                          is what gets bound. Same three shapes as `$validate`.
	 * @param string|null          $of          For an `array` property, the class each item is built from.
	 * @param array<string, mixed> $schema      Any other JSON Schema keys, merged over the type and default
	 *                                          derived from the property itself.
	 */
	public function __construct(
		public readonly string $description = '',
		public readonly mixed $validate = null,
		public readonly mixed $sanitize = null,
		public readonly ?string $of = null,
		public readonly array $schema = array()
	) {}
}
