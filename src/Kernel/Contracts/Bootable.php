 ...
 *
 * Most of a plugin only works when called: `Path` resolves a path when asked,
 * `Views` renders when asked. A few act under their own steam -- binding a hook,
 * registering a post type, walking a directory -- and those implement this.
 *
 * ```
 ...
 * level throws for one that does -- because the top level is for modules that do
 * nothing until something asks, and this one cannot keep that promise.
 *
 * ```
 * // bootstrap.php
 * return array(
...
interface Bootable {
	 ...
	 *
	 * Runs once, when the plugin builds the module -- which is what listing it in
	 * `bootstrap.php` causes.
	 *
