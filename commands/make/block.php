...
/**
 * Devtool command: `wp zestry make block <name>`.
 *
 * Generates a new editor block into a project already set up with `wp zestry
 * init`. Unlike every other `make` type this writes a directory rather than a
 * single file, since a block is a `block.json` plus the scripts, styles and
 * optional PHP it points at.
 */
...
return new class() extends MakeCommand {
	 ...
	 *
	 * Needs the `blocks` module, so run `wp zestry add module blocks` first if you
 ...
 *
 * **The slug, not the text domain.** They are answered separately and are
 * often equal, which is why this went wrong once: the module decides a block
 * is yours by comparing the namespace in its name against your slug, and
 * looks for its PHP under a `supports.{plugin-slug}-php` entry. A block namespaced
 * anything else registers, works in the editor, and renders nothing on the
 * front end.
	 *
	 * [--dir=<dir>]
